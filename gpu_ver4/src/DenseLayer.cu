...
{
    weights = new half[input_size * output_size];         // host
    biases = new half[output_size];                       // host
    weight_gradients = new half[input_size * output_size]; // host
    bias_gradients = new half[output_size];               // host

    last_input = new half[input_size * max_batch];        // host
    last_output = new half[output_size * max_batch];      // host

    // Khởi tạo weights và biases với phân phối chuẩn
    ...
    for (int i = 0; i < output_size; ++i)
    {
        ...
        biases[i] = __float2half(0.0f); // Bias là 0, chuyển đổi sang half
    }
...
void DenseLayer::backward(half *output_gradient, half *input_gradient, int batch_size)
{
    ...
    int weight_blocks = output_size;
    int weight_threads = input_size;
    size_t shared_mem_size = input_size * sizeof(half); // Shared memory for weights

    compute_weight_bias_gradients_kernel<<<weight_blocks, weight_threads, shared_mem_size>>>(d_input,
                                                                                           d_act,
                                                                                           d_weights,
                                                                                           d_wgrad,
                                                                                           d_bgrad,
    ...
    dim3 input_blocks(batch_size);
    dim3 input_threads(input_size);

    compute_input_gradient_kernel<<<input_blocks, input_threads>>>(d_act,
                                                                   d_weights,
                                                                   d_igrad,
                                                                   input_size,
                                                                   output_size,
