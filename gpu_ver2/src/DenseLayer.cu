...
void DenseLayer::forward(float* input, float* output, int batch_size) {
    ...
    // Giả sử block và grid config như trước
    int total_threads = output_size * batch_size;
    int threads = 256;
    int blocks = (total_threads + threads - 1) / threads;
    forward_kernel<<<blocks, threads>>>(d_input, d_linear_output, d_weights, d_biases, input_size, output_size, batch_size);
    ...
    if (dynamic_cast<ReLU*>(activation)) {
        ...
        // Thường softmax xử lý từng mẫu (batch_size lần gọi kernel) hoặc cải tiến kernel
...
void DenseLayer::backward(float* output_gradient, float* input_gradient, int batch_size) {
    ...
    backward_kernel<<<blocks, threads>>>(d_input, d_act, d_weights, d_wgrad, d_bgrad, d_igrad, input_size, output_size, batch_size);
    CHECK(cudaDeviceSynchronize());
