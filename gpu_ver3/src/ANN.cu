        ...
        for (int step = 0; step < steps_per_epoch; step++) {
            ...
            for (int b = 0; b < batch_size; b++) {
                for (int i = 0; i < layer3->output_size; i++) {
                    batch_target[b * layer3->output_size + i] = __float2half(0.0f);  // Chuyển đổi từ float sang half
                }
                batch_target[b * layer3->output_size + label_batch[b]] = __float2half(1.0f);  // Chuyển đổi từ float sang half
            }
