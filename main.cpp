...
int main() {
    try {
        ...
        for (int i = 0; i < image_size; ++i) {
            input[i] = image_data_train[16 + i] / 255.0f;
        }

        ANN model;
        vector<float> output = model.forward(input);

        cout << "\nOutput of ANN (Softmax Probabilities):" << endl;
        for (size_t i = 0; i < output.size(); ++i) {
            cout << "Class " << i << ": " << output[i] << endl;
        }

        auto max_pos = max_element(output.begin(), output.end());
        int predicted_label = distance(output.begin(), max_pos);
